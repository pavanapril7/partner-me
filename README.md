...
# SMS Configuration
SMS_PROVIDER="mock"  # Use "twilio" for production
...
- `SMS_PROVIDER` - SMS provider: `mock` for development, `twilio` for production
...
- `RATE_LIMIT_WINDOW_MINUTES` - Rate limit time window in minutes (default: 15)

For detailed environment configuration documentation, see:
- [Environment Configuration Guide](README_ENVIRONMENT.md) - Quick start guide
- [docs/ENVIRONMENT_CONFIGURATION.md](docs/ENVIRONMENT_CONFIGURATION.md) - Complete reference

**Configuration Validation:**

The application automatically validates all configuration on startup. If any required variables are missing or invalid, you'll see a clear error message explaining what needs to be fixed.
...
- **"Missing required environment variable: DATABASE_URL"** - Ensure `.env` file exists with `DATABASE_URL` set
- **"Invalid value for SESSION_EXPIRY_DAYS"** - Numeric values must be positive integers
- **"Invalid TWILIO_PHONE_NUMBER"** - Phone numbers must be in E.164 format (e.g., `+15551234567`)
- **SMS not working in development** - When using `SMS_PROVIDER=mock`, OTP codes are logged to the console
